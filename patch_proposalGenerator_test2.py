...
content = content.replace("store.addGovernanceCycle({", "store['db'].prepare('INSERT INTO governance_cycles (number, status, totalVotesCast, totalFundingAllocated) VALUES (?, ?, ?, ?)').run(")
content = content.replace("number: 1, status: 'ARCHIVED',", "1, 'ARCHIVED',")
content = content.replace("totalProposals: 10,\n", "")
