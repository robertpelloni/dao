...
export class ProtocolGovernance {
  ...
  async syncRoadmap(): Promise<void> {
    ...
    try {
      ...
      const findings = this.run('grep -rEn "TODO|FIXME" src/ --exclude-dir=node_modules || echo ""');
      if (findings) {
        let content = '# AUTOMATED TODO LIST\n\nGenerated by APEP Governance\n\n';
        findings.split('\n').forEach(line => {
          if (line.trim()) {
            // Clean up the grep output: file:line:  // TODO: task
            const parts = line.split(':');
            if (parts.length >= 3) {
              const file = parts[0];
              const lineNum = parts[1];
              const task = parts.slice(2).join(':').replace(/\/\/\s*|#\s*|\/\*\s*|\*\s*/g, '').trim();
              content += `- [ ] **${file}:${lineNum}** - ${task}\n`;
            } else {
              content += `- [ ] ${line.trim()}\n`;
            }
          }
